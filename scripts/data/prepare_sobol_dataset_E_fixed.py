...
Uso:
    python scripts/data/prepare_sobol_dataset.py --n-samples 25000
    python scripts/data/prepare_sobol_dataset.py --all
"""
...
def compute_features_for_case(
    ...
    Returns:
        Array shape (n_nodes, 10) com as features de cada nó
    """
    ...
    # node_indices vai de 0 a n_elements (inclusive)
    node_indices = np.arange(n_nodes)

    # x: coordenada absoluta do nó
    x = (node_indices / n_elements) * L

    # x_norm: coordenada normalizada (0 a 1)
    ...
    # Montar array de features: shape (n_nodes, 10)
    features = np.zeros((n_nodes, 10), dtype=np.float32)

    # Features globais (colunas 0-2) - replicadas para todos os nós
    features[:, 0] = I
    features[:, 1] = L
    features[:, 2] = q

    # Features derivadas (colunas 3-4) - replicadas para todos os nós
    features[:, 3] = q_scale
    features[:, 4] = q_over_EI

    # Features posicionais (colunas 5-9) - variam por nó
    features[:, 5] = x
    features[:, 6] = x_norm
    features[:, 7] = x_norm_2
    features[:, 8] = x_norm_3
...
def process_dataset(n_samples: int) -> Path:
    ...
    permitindo grafos de tamanhos variados.
    ...
    # # Filtrar apenas case_ids presentes em ambos
    # common_case_ids = set(params_df["case_id"]) & set(results_df["case_id"])
    # params_df = params_df[params_df["case_id"].isin(common_case_ids)].reset_index(drop=True)
    # results_df = results_df[results_df["case_id"].isin(common_case_ids)].reset_index(drop=True)

    # # Ordenar por case_id para garantir correspondência
    # params_df = params_df.sort_values("case_id").reset_index(drop=True)
    # results_df = results_df.sort_values("case_id").reset_index(drop=True)

    # n_samples_actual = len(results_df)
    # logger.info("Amostras em comum (usadas): {}", n_samples_actual)

    # # Validar case_ids
    # if not np.array_equal(params_df["case_id"].values, results_df["case_id"].values):
    ...
    #     I = float(params_df.iloc[idx]["I"])
    #     L = float(params_df.iloc[idx]["L"])
    #     q = float(params_df.iloc[idx]["q"])
    #     E = float(params_df.iloc[idx]["E"])
    ...
    for idx in range(n_samples_actual):
        ...
        data = Data(
            ...
            n_elements=n_elements,  # metadado adicional
        )
...
def main():
    ...
    logger.info("=" * 70)
    logger.info("PREPARAÇÃO DE DATASETS SOBOL (GRAFOS VARIÁVEIS)")
    logger.info("=" * 70)
